<template>
  <view class="wrap-min">
    ...
    <view class="list">
      ...
      <view
        class="li padtb40 pad-r20 pad-l20 ju-bt flex-a"
        @click="jump('/pages/admin/pages/coach/releaseDynamic/releaseDynamic')"
      >
        <text>个人介绍</text>
        <view class="flex">
          <u-icon name="arrow-right" color="#ccc" size="32rpx"></u-icon>
      ...
      <view
        class="li padtb40 pad-r20 pad-l20 ju-bt flex-a"
        @click="jump('/pages/admin/pages/coach/studentCase/studentCase')"
      >
        <text>学员案例</text>
        <view class="flex">
          <u-icon name="arrow-right" color="#ccc" size="32rpx"></u-icon>
      ...
      <view class="li padtb40 pad-r20 pad-l20 ju-bt flex-a" @click="show(3)">
        <text>所属医院</text>
        <view class="flex">
          <text>{{ infoData.hospital_name || "请填写" }}</text>
          <u-icon name="arrow-right" color="#ccc" size="32rpx"></u-icon>
        </view>
      </view>
      <view
        class="li padtb40 pad-r20 pad-l20 ju-bt flex-a"
        @click="jump('/pages/doctor/doctorSignature/doctorSignature')"
      >
        <text>签名</text>
        <view class="flex">
          <u-icon name="arrow-right" color="#ccc" size="32rpx"></u-icon>
    ...
    >
      <view class="modalTxt pad-b40 flex">
        ...
        <u-input
          ...
          placeholder="请输入所属医院"
          border="surround"
          v-model="infoData.hospital_name"
        ></u-input>
      </view>
    </modal-tips>
  </view>
  ...
  methods: {
    confirmFun() {
      sportsTeacherEdit({
        ...
        hospital_name: this.infoData.hospital_name,
      }).then((res) => {
        uni.showToast({
          title: res.msg,
        });
