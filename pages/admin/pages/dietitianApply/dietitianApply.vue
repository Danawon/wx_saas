...
import { dieticianApply } from '@/api/admin.js';
export default {
	data() {
		return {
			baseUrl: baseUrl,
	...
	methods: {
		// 申请营养师
		confirmApply() {
			if( !this.reason || !this.othreList.length || !this.handheldList.length || !this.reverseList.length || !this.frontList.length) {
				uni.showToast({
					title: "请输入完整信息",
					icon: "error"
				})
			...
			dieticianApply({
				reason: this.reason,
				aptitude_photo: `${this.handheldList[0].url},${this.reverseList[0].url},${this.frontList[0].url}`,
