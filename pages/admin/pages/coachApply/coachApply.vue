<template>
	<view>
		...
		<view class="pad20">
			...
			<view class="tit martb20">选择门店</view>
			<view
				class="select-box flex-row flex-a ju-bt martb40"
				@click="shopShow = true"
			>
			...
			<u-line margin="80rpx 0 0rpx 0"></u-line>
		...
		<!-- <u-action-sheet
			...
			:show="shopShow"
			cancelText="取消"
			@close="shopShow = false"
			round="20rpx"
		></u-action-sheet> -->
		<u-picker :show="shopShow" title="门店列表" :columns="shopList" @cancel="shopShow = false" @confirm="selectShop" keyName="name"></u-picker>
	</view>
</template>

<script>
import { baseUrl, baseUrlApi } from '@/utils/baseUrl.js';
import { coachApply, shopList, saleApply } from '@/api/admin.js';
export default {
	data() {
		return {
			title: "",
	...
	methods: {
		...
		selectShop(eve) {
			console.log(eve, "---ee");
			this.shopName = eve.value[0].name;
			this.shopId = eve.value[0].id;
			this.shopShow = false;
		},
		// 获取门店列表
		getShopList() {
